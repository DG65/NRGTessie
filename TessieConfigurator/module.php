...
class TessieConfigurator extends IPSModule
{
    ...
    public function GetConfigurationForm(): string
    {
        ...
        $elements = [
            ...
            ['type' => 'Label', 'label' => 'Symcon 9.0: Create-Chain Reihenfolge Vehicle → WS (I/O darf keinen Parent haben).'],
            ['type' => 'Label', 'label' => 'Telemetrie nutzt ?access_token=... (keine Custom-Headers).']
        ];
    ...
    private function buildCreateChain(string $vin, string $name, string $token): array
    {
        ...
        // ✅ Symcon 9.0: Vehicle first, then I/O
        return [
            ['moduleID' => self::VEHICLE_MODULE_ID, 'configuration' => $vehicleCfg, 'name' => $name],
            ['moduleID' => self::WS_CLIENT_MODULE_ID, 'configuration' => $wsCfg, 'name' => 'Tessie Telemetry ' . $vin]
        ];
    }
    ...
    private function fetchVehicles(string $token): array
    {
        ...
        if (isset($data['results']) && is_array($data['results'])) {
            ...
            foreach ($data['results'] as $row) {
                ...
                $last = $row['last_state'] ?? [];
                $dn = '';
                if (is_array($last)) $dn = (string)($last['display_name'] ?? ($last['vehicle_state']['vehicle_name'] ?? ''));
