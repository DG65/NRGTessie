...
[![Check Style](https://github.com/DG65/NRGTessie/actions/workflows/check-style.yml/badge.svg)](https://github.com/DG65/NRGTessie/actions/workflows/check-style.yml)
...
> Teil des **NRG-Stack** — dem Energie-Modulverbund von DG65. Das Modul ist auch eigenständig voll nutzbar.
...
- **`stat_tel_DoorState`** (Datenpunkt „Tür-/Klappenstatus"): kommagetrennte Liste der **aktuell offenen** Türen/Klappen, leerer Wert = alles geschlossen. Mögliche Einträge: „Fahrertür vorn", „Tür hinten links", „Beifahrertür vorn", „Tür hinten rechts", „Frunk" (vorderer Kofferraum), „Kofferraum" (Heckklappe) – live gegen ein echtes Fahrzeug verifiziert. Relevant für Konsumenten, die automatisch reagieren wollen: `act_front_trunk`/`act_rear_trunk` senden Teslas reinen Umschalt-Befehl ohne Richtung (kein „schließen", nur „auslösen") – vor einem automatischen Auslösen sollte daher immer erst per `stat_tel_DoorState` geprüft werden, ob die Klappe überhaupt offen ist. Der Frunk lässt sich über die Tesla-API grundsätzlich nicht schließen, nur öffnen – bleibt also nach dem Öffnen so lange in der Liste, bis er von Hand geschlossen wird.
