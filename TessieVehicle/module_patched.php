...
class TessieVehicle extends IPSModule
{
    ...
    public function RequestAction($Ident, $Value)
    {
        ...
        switch ((string)$Ident) {
            ...
            case self::ACT_TEMP_PASSENGER:
                ...
                if ($temp < 15) $temp = 15;
                if ($temp > 28) $temp = 28;
                // Tessie API setzt eine Zieltemperatur (Celsius). Wir halten beide Setpoints synchron.
    ...
    {
        // Nur numerische Werte umrechnen
    ...
    private function syncFromTelemetry(array $dataItems): void
    {
        ...
        foreach ($dataItems as $item) {
            ...
            if ($key === 'ValetModeEnabled') {
                $b = $this->telemetryGetBool($val);
                if ($b !== null) {
                    $this->safeSetValueIfExists(self::ACT_VALET_MODE, $b);
                }
                continue;
            }
            if ($key === 'SentryMode') {
                // z.B. SentryModeStateOff / SentryModeStateOn
            ...
            if (array_key_exists('locationValue', $val) && is_array($val['locationValue'])) {
                ...
                if (!isset($registry[$identLat])) {
                    $registry[$identLat] = $this->makeRegistryEntry($key, $this->translateTelemetryKey($key) . ' – Breitengrad', VARIABLETYPE_FLOAT);
                    $registryChanged = true;
                }
                if (!isset($registry[$identLon])) {
                    $registry[$identLon] = $this->makeRegistryEntry($key, $this->translateTelemetryKey($key) . ' – Längengrad', VARIABLETYPE_FLOAT);
                    $registryChanged = true;
                }
    ...
    {
        // Häufige Keys (Rest bleibt original)
        $map = [
            'Soc' => 'Ladestand (SoC)',
            'BatteryLevel' => 'Ladestand (SoC)',
            'EnergyRemaining' => 'Restenergie',
            'RatedRange' => 'Reichweite (Rated)',
            'IdealBatteryRange' => 'Reichweite (Ideal)',
            'EstBatteryRange' => 'Reichweite (Geschätzt)',
            'ChargeState' => 'Ladezustand',
            'DetailedChargeState' => 'Ladezustand (Detail)',
            'InsideTemp' => 'Innentemperatur',
            'OutsideTemp' => 'Außentemperatur',
            'Odometer' => 'Kilometerstand',
            'GpsHeading' => 'Fahrtrichtung (GPS)',
            'Location' => 'Standort'
            'CurrentLimitMph' => 'Geschwindigkeitslimit (km/h)',
            'VehicleSpeed' => 'Geschwindigkeit (km/h)',
            'MilesToArrival' => 'Entfernung bis Ziel (km)',
            'TimeToFullCharge' => 'Zeit bis voll',
            'MinutesToArrival' => 'Minuten bis Ziel',
            'ChargePortDoorOpen' => 'Ladeport offen',
            'SentryMode' => 'Wächtermodus (Sentry)',
            'ValetModeEnabled' => 'Valet-Modus',
            'DefrostMode' => 'Defrost-Modus',];
        return $map[$key] ?? $key;
    }
    ...
    private function guessProfileForTelemetryKey(string $key, int $type): string
    {
        ...
        if ($type !== VARIABLETYPE_STRING && (strpos($k, 'soc') !== false || strpos($k, 'percent') !== false)) {
            return ($type === VARIABLETYPE_INTEGER) ? 'Tessie.PercentInt' : 'Tessie.PercentInt';
        }
        ...
        // Geschwindigkeit kommt als mph, wir speichern km/h
        if ($type !== VARIABLETYPE_STRING && strpos($k, 'mph') !== false) {
            return 'Tessie.SpeedKmh';
        }

        // Distanzen/Ranges kommen als Miles, wir speichern km
    ...
    private function ensureLinkTree(bool $forceRename = false): void
    {
        ...
        $domain = [
            self::PURPOSE_CHARGING => [
                ...
                self::ACT_OPEN_CHARGE_PORT,
                self::ACT_CLOSE_CHARGE_PORT,
            ...
            self::PURPOSE_CLIMATE => [
                self::ACT_CLIMATE,
                self::ACT_TEMP_DRIVER,
                self::ACT_TEMP_PASSENGER,
                self::ACT_DEFROST,
                self::ACT_STEERING_WHEEL_HEATER,
                self::ACT_SEAT_HEAT_DRIVER,
                self::ACT_SEAT_HEAT_PASSENGER
            ],
            self::PURPOSE_SECURITY => [
                self::ACT_LOCKED,
                self::ACT_SENTRY_MODE,
                self::ACT_VALET_MODE,
                self::ACT_FLASH,
                self::ACT_HONK,
                self::ACT_VENT_WINDOWS,
                self::ACT_CLOSE_WINDOWS
            ]
        ];
