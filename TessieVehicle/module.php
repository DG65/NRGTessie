...
class TessieVehicle extends IPSModule
{
    ...
    public function Create(): void
    {
        ...
        // Auto-Variablen: Metric ist Standard, RAW optional
        ...
        $this->RegisterAttributeString('DetectedUnitSystem', ''); // metric|imperial
    ...
    public function ApplyChanges(): void
    {
        ...
        // Kategorien: Metric ist Standard, RAW optional
        ...
        // Einheiten möglichst früh ermitteln (damit Telemetrie sofort korrekt umgerechnet wird)
    ...
    public function Update(): void
    {
        ...
        if (is_array($state) && count($state) > 0) {
            ...
            // Auto-Variablen: Metric zuerst (Standard), RAW optional
            if ((bool)$this->ReadPropertyBoolean('AutoMetric')) {
    ...
    public function ReceiveData($JSONString): void
    {
        ...
        // WICHTIG: Bei Telemetrie kann Update() noch nicht gelaufen sein.
        // Daher Einheiten hier sicherstellen, bevor wir metric schreiben.
        $this->ensureDetectedUnitSystem();
    ...
    private function ensureDetectedUnitSystem(): void
    {
        ...
        // Auto: versuche einmalig per REST State die gui_settings zu lesen.
        $token = trim((string)$this->ReadPropertyString('ApiToken'));
        $vin = trim((string)$this->ReadPropertyString('VIN'));
        if ($token === '' || $vin === '') {
        ...
        // Fallback: wenn immer noch unbekannt, metric annehmen
    ...
    {
        // Tessie Get Vehicle liefert gui_settings.gui_distance_units z.B. "mi/hr" und gui_temperature_units z.B. "F".
        $gui = $state['gui_settings'] ?? null;
        if (is_array($gui)) {
            $dist = strtolower((string)($gui['gui_distance_units'] ?? ''));
            $temp = strtoupper((string)($gui['gui_temperature_units'] ?? ''));
            if (strpos($dist, 'mi') !== false || $temp === 'F') {
                return 'imperial';
            }
            if (strpos($dist, 'km') !== false || $temp === 'C') {
                return 'metric';
            }
        }
        return '';
