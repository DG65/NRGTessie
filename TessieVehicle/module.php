...
class TessieVehicle extends IPSModule
{
    ...
    public function Create()
    {
        ...
        // Default: newly discovered telemetry vars are disabled in the list
    ...
    // Dynamic single-list form with telemetry entries merged in
    ...
    // Telemetry sync: keeps existing mapped keys + auto-discovery into registry
    private function syncFromTelemetry(array $dataItems): void
    {
        ...
        foreach ($dataItems as $item) {
            ...
            // Existing mappings (same as your original module)
            if ($key === 'Locked') {
            ...
            if (array_key_exists('locationValue', $val) && is_array($val['locationValue'])) {
                ...
                // update values only if variables exist (enabled)
    ...
    private function ensureVariables(): void
    {
        ...
        // Telemetry vars from registry: only if user enabled them in VisibleVars
        $registry = $this->getTelemetryRegistry();
        foreach ($registry as $ident => $meta) {
            if (!is_string($ident) || $ident === '') continue;
            if (!is_array($meta)) continue;

            // If not in enabled map yet: treat as disabled
    ...
    // -------- Link tree (based on original, plus telemetry domain links) --------
    private function ensureLinkTree(bool $forceRename = false): void
    {
        ...
        // Telemetry domain links based on key classification
        $registry = $this->getTelemetryRegistry();
        foreach ($registry as $ident => $meta) {
            if (!is_string($ident) || $ident === '') continue;
